...
declared as `TASK-INFERENCE` under DIMER Notebook Specification 1.1 and **standalone** (§3.6): generated by
`tools/build_notebook.py`, it carries the package's seven modules, the model identity, the manifest digests and the
runtime pins, so the exported notebook runs without this repository (parity enforced by
`tests/test_notebook_parity.py` and `tools/validate_release_assets.py`). It runs the same public Python API as the
package and covers:

1. pinned-runtime install and runtime identity (no clone, no repository install);
2. the carried package, then pinned `amazon/chronos-2` acquisition at the immutable revision `95a9710e2596287d08352589f42634fa5abdf0a7` with manifest digest verification;
3. the deterministic synthetic sample regenerated in code (digest-asserted against the checked-in CSV) or BYOD CSV with duplicate-header defense;
4. chronological future holdout with no target leakage, the operational ceilings, and `validate_inputs` → input manifest;
5. zero-shot median + quantile forecast;
6. chronological evaluation against last-value and guarded seasonal-naive baselines, and `evaluation_report` → evaluation report;
...
The real-weight CI job executes the notebook's code cells sequentially on the locked environment and uploads the
generated `outputs/` as workflow evidence (default path, then the BYOD-shaped and optional Mode D paths). It is a
pre-flight on the locked stack, not a fresh-boundary run of the inline pins; static notebook JSON/source checks alone are
not treated as live-readiness evidence. See `tutorials/README.md` for the registry and `docs/release-verification.md`
for the release gate.

## Release status

**Candidate.** Static/unit checks do not constitute clean-runtime notebook evidence. The clean-runtime run of the
standalone tutorial is pending; complete `docs/release-verification.md` against the exact release revision before
calling the notebook release-grade. See `STATUS.md`.
