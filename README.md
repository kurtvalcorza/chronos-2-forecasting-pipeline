...
declared as `TASK-INFERENCE` under DIMER Notebook Specification 1.0. It runs the same public
Python API as the package and covers:

1. locked-runtime bootstrap and runtime identity;
2. bundled deterministic sample or BYOD CSV with duplicate-header defense;
...
6. chronological evaluation against last-value and guarded seasonal-naive baselines;
7. primary **Mode C multi-target forecasting** with target-name alignment assertion;
8. machine-readable forecast, metrics, and provenance export;
9. optional Mode D known-future numeric covariates; and
10. explicit interpretation/evidence limits.

The real-weight CI job executes the notebook code path and uploads generated `outputs/` as workflow
evidence. It includes a clean-directory bootstrap path plus BYOD-shaped and optional Mode D paths;
static notebook JSON/source checks alone are not treated as live-readiness evidence. Hosted Colab
is an external substrate, so a release-grade Colab claim still requires a recorded clean-Colab run
for the exact release revision.
...
`season_length`; the tutorial only uses its 24-step hourly comparator when every series has enough
post-holdout history, otherwise it skips that optional baseline.
...
The source of truth is `generate_samples.py`. `SHA256SUMS` contains only tutorial artifacts that
are actually present in a clean checkout and is directly verifiable there;
`SHA256SUMS.generated` records the canonical digests of generated-only multi-series and covariate
examples. Tests regenerate all sample artifacts and verify their identities and DIMER validation
semantics.
...
tutorials/README.md                 notebook profile / release registry
tutorials/chronos_2_forecasting_colab.ipynb
tools/run_notebook.py               executable notebook path
