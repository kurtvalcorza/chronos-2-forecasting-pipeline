...
# INF1: the exact load expression the model cell must use. This package's documented loader is the
# function `load_pinned_model` (it returns a `LoadedModel`); the template's `model_load` names it.
MODEL_LOAD_EXPR = "load_pinned_model(weights_dir=WEIGHTS_DIR)"
# Additional 40-hex revisions a document may legitimately cite: the reviewed upstream
# chronos-forecasting v2.3.1 library commit named in MODEL_CARD.md's provenance table.
KNOWN_SHAS: frozenset[str] = frozenset(("7dc4435706a4454feb79df44ca9f33631f3027bf",))
...
EXPECTED_OUTPUTS = (
    "outputs/chronos_2_forecasting_input_manifest.json",
    "outputs/chronos_2_forecasting_evaluation_report.json",
    "outputs/chronos_2_forecasting_result.json",
    "outputs/chronos_forecast.csv",
    "outputs/chronos_multitarget_forecast.csv",
    "outputs/chronos_evaluation.json",
    "outputs/chronos_provenance.json",
)
...
CODE_MARKERS = (
    ...
    "seasonal_naive_baseline(split.history, split.truth, config, season_length=24)",
    ...
    "SAMPLE_SHA256 = \"eff96b1a9bec5a81aa4021b4d308c29e18fc89be0cf8eb54e755c2efe535c7f7\"",
...
MARKDOWN_MARKERS = (
    "**Capability:** zero-shot probabilistic time-series forecasting",
    "**No training or fine-tuning occurs**",
    "**no adaptation occurs:**",
    "`prediction` is the **median (`q0.5`)**, not a mean",
    "**not guaranteed frequentist confidence intervals**",
    "the pipeline ships no threshold",
    "**no random split**",
    "**MAE**",
    "**RMSE**",
    "**pinball loss**",
    ...
    "`sample-sanity`",
    "**every series has at least 24 post-holdout history rows**",
    "Primary Mode C",
...
def validate_model_card() -> None:
    ...
    _check(
        "## Checkpoint and runtime provenance" in text,
        "MODEL_CARD.md must carry its '## Checkpoint and runtime provenance' section (this card's immutable-provenance record)",
    )
