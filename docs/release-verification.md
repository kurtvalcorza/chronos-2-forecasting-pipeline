...
reads `clean_runtime_evidence: pending`, because no single blob yet has passing runs of every path,
and changing the metadata would change the blob these runs verify.

The `eb426b9` revision rewrote the guided text and cell titles only; its `STANDARD` runs above cover both samples at that blob. `4199536` (blob `2213095d1e58`) changes one line of runner environment setup (`MPLBACKEND=Agg`); its `FULL` run above passed with the Open-Meteo sample and exercises all three models. A `STANDARD` run of the synthetic default path at that blob has not been recorded, so the notebook metadata still reads `clean_runtime_evidence: pending`.
